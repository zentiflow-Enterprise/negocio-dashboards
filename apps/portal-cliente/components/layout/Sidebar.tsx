    ...
    return (
        <aside className={`${mobile ? "flex" : "hidden md:flex"} w-64 border-r border-[var(--border)] bg-[var(--bg-soft)] p-5 flex-col min-h-screen`}>
            ...
            <nav className="flex-1 flex flex-col gap-9">
                {NAV_ITEMS.map((group) => (
                    <div key={group.section}>
                        <p className="text-[10px] text-[var(--text-soft)] uppercase tracking-widest px-3 mb-3">
                            {group.section}
                        </p>
                        <div className="flex flex-col gap-1">
                            {group.items.map((item) => (
                                <Link
                                    key={item.href}
                                    href={item.href}
                                    className={`flex items-center gap-3 px-4 py-3 rounded-2xl text-[16px] font-medium transition-all ${isActive(item.href)
            ...
            <div className="border-t border-[var(--border)] pt-6 mt-auto">
                ...
                <button
                    onClick={logout}
                    className="mt-4 w-full py-3 rounded-2xl text-red-500 hover:bg-red-500/10 hover:text-red-600 transition-all text-sm font-medium"
                >
                    Cerrar sesión
                </button>
